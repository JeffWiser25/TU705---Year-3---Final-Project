...
class STM32SpeedApp:
    ...
    def build_ui(self):
        ...
        ttk.Label(ctrl, text=f"Setpoint RPM (0-{MAX_SETPOINT_RPM})").grid(row=3, column=0, padx=5, pady=5)
        ...
        ttk.Label(ctrl, text=f"Manual PWM (0-{MAX_MANUAL_PWM})").grid(row=4, column=0, padx=5, pady=5)
    ...
    def send_sp(self):
        try:
            value = int(float(self.sp_entry.get().strip()))
            if value < 0:
                value = 0
            if value > MAX_SETPOINT_RPM:
                value = MAX_SETPOINT_RPM
                self.sp_entry.delete(0, tk.END)
                self.sp_entry.insert(0, str(value))
                messagebox.showinfo("Setpoint Limited", f"Setpoint capped to {MAX_SETPOINT_RPM} RPM.")
            self.send_cmd(f"SP {value}")
        except ValueError:
            messagebox.showerror("Input Error", "Enter a valid setpoint RPM.")

    def send_pwm(self):
        try:
            value = int(float(self.pwm_entry.get().strip()))
            if value < 0:
                value = 0
            if value > MAX_MANUAL_PWM:
                value = MAX_MANUAL_PWM
                self.pwm_entry.delete(0, tk.END)
                self.pwm_entry.insert(0, str(value))
                messagebox.showinfo("PWM Limited", f"Manual PWM capped to {MAX_MANUAL_PWM}.")
            self.send_cmd(f"PWM {value}")
        except ValueError:
            messagebox.showerror("Input Error", "Enter a valid PWM value.")

    def send_gains(self):
        try:
            kp = float(self.kp_entry.get().strip())
            ki = float(self.ki_entry.get().strip())
            kd = float(self.kd_entry.get().strip())
            ...
            messagebox.showerror("Input Error", "Enter valid gain values.")
    ...
    def handle_data(self, line):
        try:
            ...
            _, sp, rpm, err, pwm, kp, ki, kd, mode, enabled = parts
