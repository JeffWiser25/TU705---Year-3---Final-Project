...
class STM32SpeedApp:
    ...
    def build_ui(self):
        ...
        ttk.Button(tune, text="Send Gains", command=self.send_gains).grid(
            row=3, column=0, columnspan=2, padx=5, pady=8
        )
    ...
    def send_cmd(self, cmd):
        if self.ser and self.ser.is_open:
            try:
                ...
                self.var_status.set(f"TX error: {e}")
        ...
        while self.running:
            try:
                ...
                if line.startswith("DATA,"):
                    self.root.after(0, self.handle_data, line)
            except Exception:
                break
